...
import type { AllEmojis, ValidCodepoints, ValidCategories } from '@/models/emojis'
...
export function loadEmojis() {
  ...
  for (const [codepoint, emojiData] of Object.entries(allEmojis)) {
    state.allEmojis.set(Number(codepoint), emojiData)
  }
}
...
export function moveLeft(position: number) {
  ...
  if (position > 0) {
    const currentEmoji: number | undefined = state.stagingEmojis[position]
    const leftEmoji: number | undefined = state.stagingEmojis[position - 1]
...
export function moveRight(position: number) {
  ...
  if (position < state.stagingEmojis.length) {
    const currentEmoji: number | undefined = state.stagingEmojis[position]
    const rightEmoji: number | undefined = state.stagingEmojis[position + 1]
